...
function isNearRock(x: number, z: number) {
  return ROCK_FORMATIONS.some((rock) => {
    const dx = x - rock.position[0];
    const dz = z - rock.position[2];
    const clearX = rock.collider[0] + GRASS_ROCK_CLEARANCE;
    const clearZ = rock.collider[2] + GRASS_ROCK_CLEARANCE;
    if (Math.abs(dx) < clearX && Math.abs(dz) < clearZ) {
      return true;
    }
    const radialClear = Math.max(clearX, clearZ);
    return dx * dx + dz * dz < radialClear * radialClear * 0.62;
  });
}
...
export function WorldGeometry() {
  ...
  const grassField = useMemo(() => {
    ...
    const maxAttempts = GRASS_BLADE_COUNT * 24;
    let bladeCount = 0;
    for (let attempt = 0; attempt < maxAttempts && bladeCount < GRASS_BLADE_COUNT; attempt++) {
      ...
      const density = THREE.MathUtils.clamp(
        macroDensity * 0.72 + patchDensity * 0.28,
        0,
        1,
      );
      ...
      const height =
        GRASS_BLADE_BASE_HEIGHT *
        heightNoise *
        THREE.MathUtils.lerp(0.86, 1.14, density);

      const yaw = hash1D(attempt * 9.1 + 2.2) * TAU;
      const leanX = (hash1D(attempt * 11.7 + 6.6) - 0.5) * 0.08;
      const leanZ = (hash1D(attempt * 12.4 + 7.1) - 0.5) * 0.08;
      rotation.set(leanX, yaw, leanZ);
      quaternion.setFromEuler(rotation);

      basePosition.set(x, GRASS_BASE_Y, z);
...
`,
  ...
  }, [grassField.bladeGeometry, grassMaterial, rockGeometries, rockMaterial, rockNoiseTexture]);
