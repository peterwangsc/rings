...
- Files changed: `app/gameplay/collectibles/Ring.tsx`, `app/utils/constants.ts`, `docs/v1-multiplayer-platform-plan.md`.
- Validation: `npx eslint app/gameplay/collectibles/Ring.tsx app/utils/constants.ts` (pass), `npm run build` (pass), `npm run lint` (fails on pre-existing unrelated errors/warnings in `docs/*`, `spacetimedb/dist/*`, `.next/*`, and `v2/*` paths).
- User-requested multiplayer identity polish: replaced autogenerated `Guest-XXXXXX` defaults with curated one-word guest names for new arrivals in `app/multiplayer/spacetime/client.ts` via `app/multiplayer/spacetime/guestDisplayNames.ts`.
- Added migration behavior for previously persisted legacy guest IDs (`Guest-XXXXXX` and `Guest-local`) so reconnecting users are reassigned to the new colloquial name bank.
- Updated server-side empty display-name sanitization fallback to deterministic identity-seeded curated names in `spacetimedb/src/validation/reducerValidation.ts` via `spacetimedb/src/shared/guestDisplayNames.ts`, preserving reducer determinism.
- Roadmap status: `Phase 3 / Separate inbound ingestion, reconciliation, and outbound dispatch responsibilities in V1 net code` remains blocked for full completion because this session was explicitly scoped to user-requested guest-name assignment polish; next action is to resume the Phase 3 networking ownership split.
- Files changed: `app/multiplayer/spacetime/client.ts`, `app/multiplayer/spacetime/guestDisplayNames.ts`, `app/scene/CharacterRigScene.tsx`, `spacetimedb/src/validation/reducerValidation.ts`, `spacetimedb/src/shared/guestDisplayNames.ts`, `docs/v1-multiplayer-platform-plan.md`.
- Validation: `npx eslint app/multiplayer/spacetime/client.ts app/multiplayer/spacetime/guestDisplayNames.ts app/scene/CharacterRigScene.tsx spacetimedb/src/validation/reducerValidation.ts spacetimedb/src/shared/guestDisplayNames.ts` (pass), `npm run build` (pass), `npm run multiplayer:db:publish` (pass), `npm run multiplayer:db:generate` (pass), `npm run lint` (fails on pre-existing unrelated errors/warnings in `docs/*`, `spacetimedb/dist/*`, `.next/*`, and `v2/*` paths).
